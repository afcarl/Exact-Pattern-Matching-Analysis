...
In order to verify the theoretical analysis, we generated some experimental results. The plot in figure \ref{bmpreprocess} shows that the preprocessing time of Boyer Moore increases linearly to the pattern length. This is in complete agreement with the theoretical preprocessing complexity of the algorithm, since the time required to construct the table is proportional to the length of the pattern O(k*|Σ|).


\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{figures/bm-preprocess.png}
\caption{Preprocessing time of Boyre Moore as the pattern length increases}
\label{bmpreprocess}
\end{figure}


Figure \ref{bmtext} shows the behavior of our algorithm for two different patterns, one that occurs frequently and one that does not occur, as the text length increases. As expected Boyre Moore algorithm illustrates better behavior when the pattern does not occur in the text length, since the length of the shifts that are performed in the text is larger. Indeed in some cases that the pattern does not exist in the text, the algorithm presents sub linear complexity.

\begin{figure}[h!]
\centering
\includegraphics[width=1\textwidth]{figures/bm-search.png}
\caption{Searching time of Boyre Moore as the text length increases}
\label{bmtext}
\end{figure}

Finally, we study the behavior of Boyre Moore algorithm for a fixed size text, as the pattern length gradually increases. The second plot in figure \ref{bmpattern} that regards the natural text, the searching time continually decreases as the pattern length increases. This is normal and expected, since the longer patterns do not appear in the text, and therefore the algorithm is able to perform larger shifts on the text. However this does not happen with DNA sequence, where a worse behavior is illustrated, verifying that Boyer Moore does not perform well in small length alphabets.

\begin{figure}[h!]
\centering
\includegraphics[width=1\textwidth]{figures/bm-search2.png}
\caption{Searching time of Boyre Moore as the pattern length increases}
\label{bmpattern}
\end{figure}
