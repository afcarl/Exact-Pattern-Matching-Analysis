...
The first experiment was to observe the preprocessing time complexity of O($k$). This was done by fixing the text length and varying the pattern length expecting to observe linear curve in figure~\ref{fig:pre-fixed-text}. The observed curve shows the linear relationship and appears to fluctuate between 0 and 1. The y-axis here is measured against ticks which is 10$ms$. This measure is quite sensitive due to the unity. The longest length of pattern was the length of 100 characters.
