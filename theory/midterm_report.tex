...
First, we need to introduce some additional notations, and a lemma without a proof (the proof is essential and completely based on considering two different cases of the algorithm for an $STrie$ construction). Lets denote all the states corresponding to suffixes in the currently processed prefix $T^i$ by $\bar s_1, \bar s_2, \dots, \bar s_i$ where $\bar s_h = \overline{t_1 t_2 \dots t_{i - h + 1}}$.
...
Finally, we need to consider the procedure of adding transitions to states $s_h$ along the boundary path between the active point and the end point ($j \le h < j'$) splitting the nodes. We start from the state $s_j$ (the active point). Lets denote the canonical reference pair of the active point by $bar s, w)$, which is by definition (see lemma~\ref{lemma:active-end-point}) is the same as $(\bar s, (k, i-1))$. We first check, if the end point coincides with the canonical point. If yes, we are done. Otherwise, we should perform create a new branch, splitting the current state if it is implicit. The newly created transition will be $g'(\bar s_h, (i, \infty)) = \bar s'_h$ where $\bar s'_h$ is a new leaf. Also, if $\bar s_h$ was implicit and became explicit by splitting, a new suffix function link $f'(s_h)$ should be added. Then we advance the construction to the next state $\bar s_{h+1}$.\\

Before we start working with the next state, we need to make sure that its reference pair is canonical. For this purpose, we use procedure \textit{canonize}, which updates the reference pair of the current state to the canonical representation \footnote{To see how exactly work this procedure (along with some additional subtle moments of the algorithm) please refer to the original paper~\cite{ukkonen1995line}. In this report, we decided to omit some of the minor, but important points for the sake of space.}.\\
...
The main theorem that was stated in~\cite{ukkonen1995line} is
...
Suppose that we have a suffix tree of a text $banana$, as shown in figure~\ref{fig:suffix-tree-ex}. The length of text is 6, so it has 7 leaves. Each leaf represents a suffix of the text. From figure~\ref{fig:suffix-tree-ex}, there are 7 suffixes, such as $\$$, $a\$$, $na\$$, $ana\$$, $nana\$$, $anana\$$, $banana\$$.
...
\par For a given pattern $Q$, the algorithm will follow a unique path down from root of $T$ according to characters in $Q$. If all characters of $Q$ is found to be a prefix of such a path, then return the labels of some leaves below this path. If there comes a mismatch, then return no match found. For example, for a given pattern $Q = \{nan\}$ and a given pattern $Q = \{anab\}$, the algorithm works as shown in figure~\ref{fig:suffix-tree-matching-ex}.
